...
export default function MyHotelBookingsReceivedSection() {
  ...
  const load = async () => {
    setBusy(true);
    try {
      const res = await api.get("/my/hotel-bookings");
      setItems(res.data.items || []);
    } catch (e) {
      setItems([]);
      toast(e?.response?.data?.message || "Failed to load received bookings", 2500);
    } finally {
      setBusy(false);
    }
  };
  ...
  const setStatus = async (b, nextStatus) => {
    ...
    try {
      await api.patch(`/my/hotel-bookings/${b._id}/status`, { status: nextStatus });
  ...
  const verifyPayment = async (b, decision) => {
    ...
    try {
      await api.patch(`/my/hotel-bookings/${b._id}/payment/verify`, {
        decision,
        note: "",
      });
    ...
    <div className="card" style={{ boxShadow: "none", marginTop: 12 }}>
      <div className="cardBody">
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <div>
            <h4 style={{ margin: "0 0 6px" }}>Bookings Received (My Hotels)</h4>
        ...
        {items.length === 0 && !busy ? (
          <div className="p">No bookings received yet.</div>
        ) : (
                  ...
                  <div className="cardBody" style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    <div style={{ minWidth: 320 }}>
                      ...
                      <div className="p" style={{ fontSize: 12 }}>
                        Room: <b>{b.room?.roomName || "—"}</b> • Total: <b>{money(b.total)}</b>
                      </div>
                      <div className="p" style={{ fontSize: 12 }}>
                          ...
                          <span style={{ display: "inline-flex", gap: 8, flexWrap: "wrap" }}>
                            {proofs.map((p) => (
                              <a
                                ...
                                href={`${api.defaults.baseURL}/my/hotel-bookings/${b._id}/proofs/${p._id}`}
                                target="_blank"
                                rel="noreferrer"
                              >
                                Download
