 ...
 * - onViewAll?: () => void
 */
export default function MyGuideBookingsSection({
  limit,
  hideHeader = false,
  compact = false,
  headerLabel = "My Guide Bookings",
  onViewAll,
}) {
  ...
  const load = async () => {
    setBusy(true);
    try {
      const res = await api.get("/my/bookings/guide");
      setItems(res.data.items || []);
    } catch (e) {
      setItems([]);
      toast(e?.response?.data?.message || "Failed to load guide bookings", 2500);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="card" style={{ boxShadow: "none" }}>
      <div className="cardBody">
        {!hideHeader ? (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              <div>
                <h3 style={{ margin: "0 0 6px" }}>My Guide Bookings</h3>
        ...
        {items.length === 0 && !busy ? (
          <div className="p">No guide bookings yet.</div>
        ) : (
                    ...
                    <div className="cardBody" style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                      <div>
                        ...
                        <div className="p" style={{ fontSize: 12, marginTop: 6 }}>
                          {fmtDate(b.startDate)} → {fmtDate(b.endDate)} • {b.days} days • Travelers: {b.travelers}
                        </div>
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                        {proofs.length > 0 ? (
                          <a
                            ...
                            href={`${api.defaults.baseURL}/my/bookings/guide/${b._id}/proofs/${proofs[0]._id}`}
                            target="_blank"
                            rel="noreferrer"
                          >
                            Proof
                        ...
                        {needsProof ? (
                          <Link
                            ...
                            style={{ padding: "6px 10px", fontSize: 12, borderRadius: 999 }}
                            to={`/submit-payment?type=GUIDE_BOOKING&id=${b._id}&method=${defaultMethod}`}
                          >
                            Submit Proof
                          </Link>
                        ) : (
                  ...
                  <div className="cardBody" style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    <div style={{ minWidth: 320 }}>
                          ...
                          <span style={{ display: "inline-flex", gap: 8, flexWrap: "wrap" }}>
                            {proofs.map((p) => (
                              <a
                                ...
                                href={`${api.defaults.baseURL}/my/bookings/guide/${b._id}/proofs/${p._id}`}
                                target="_blank"
                                rel="noreferrer"
                              >
                                Download
                      ...
                      {needsProof ? (
                        <div style={{ marginTop: 10 }}>
                          <Link className="btn primary" to={`/submit-payment?type=GUIDE_BOOKING&id=${b._id}&method=${defaultMethod}`}>
                    ...
                    <div style={{ minWidth: 260 }}>
                      <div className="p" style={{ fontSize: 12 }}>
                        <b>Meeting City:</b> {b.meetingCity || "—"}
                      </div>
                      <div className="p" style={{ fontSize: 12 }}>
