...
router.get("/", requireAuth, async (req, res) => {
  ...
  const items = await GuideBooking.find(filter).sort({ createdAt: -1 });
  res.json({ items });
});
...
router.get("/:id", requireAuth, async (req, res) => {
  const userId = req.auth.userId;
  const item = await GuideBooking.findOne({ _id: req.params.id, userId });
...
router.get("/:id/proofs/:proofId", requireAuth, async (req, res) => {
  ...
  const booking = await GuideBooking.findOne({ _id: id, userId });
